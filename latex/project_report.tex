...
For an OFDM radar, we can solve for the unambiguous range and velocity using Equations (\ref{eq::unambig_range}), (\ref{eq::unambig_velocity}), and (\ref{eq::ofdm_radar_prf}). This leads to the following results:
...
\begin{equation}
	v_{ua} \approx 1589\ \text {m/s}
\end{equation}

Due to the demonstrated performance degradations at large ranges and velocities, it is not meaningful to evaluate performance outside of the first range and velocity ambiguity. As a result, we do not attempt to measure the unambiguous range and velocity. In the 802.11p radar is range and velocity-limited well within the first ambiguities. However, even with this limitation it still will be able to measure an acceptable set of ranges and velocities.

We can now examine the range resolution of the system. The range resolution is the minimum separation at which two objects are resolvable. For this experiment, we disable the range window to get the finest possible range resolution and zero-pad the IFFT to increase our visibility of the resulting range response. In Figure \ref{fig::ofdm_radar_range_resolution}, we show the range response of two objects separated by 30 m.
...
Examining the above figure, we find that the mainlobe width is $36.186\ \text{m}$. Our measured range resolution is then $36.186\ \text{m} /2 = 18.093 \ \text{m}$. For reference, we can compute the range resolution of the system using Equation (\ref{eq::range_resolution}) with a bandwidth $\beta = 53/64 \times 10\ \text{MHz} = 8.281\ \text{MHz}$. This results in a range resolution of $18.1\ \text{m}$, which is consistent with the measured data.

Note that this range resolution is significantly larger than it was for the FMCW radar. In fact, the range resolution of the 802.11p radar is insufficient for automotive radar applications. However, we can design an OFDM system with more bandwidth in the 77 GHz band that will achieve a better range resolution. 
...
The mainlobe width is now 1.1527 m, which corresponds to a range resolution of 0.5763 m. This is consistent with the theoretical result. It is important to note that this range resolution will further degrade after windowing is applied. However, for automotive applications, the updated parameters provide a much more acceptable range resolution.
...
where $N$ is the number of subcarriers and $L_{fast}$ is the SNR loss due to the fast-time window. For the given system with an 80 dB Chebyshev window, the fast time SNR gain is approximately given by

\begin{equation*}
	G_{fast} \approx 1175.54
\end{equation*}

Using Equation (\ref{eq::fmcw_fast_time_snr_gain}) with a PRF of $320\ \text{MHz}/(80 \times 16) = 125\ \text{kHz}$, we compute $1469.56$ as the equivalent FMCW SNR gain. Note that FMCW SNR gain is greater than the OFDM Radar SNR gain by approximately $(N + L)/N$. For the given OFDM parameters, that works out to be roughly 0.97 dB. Therefore, we expect the SNR in the RDM to be approximately 13 dB lower for the JRC system than it is for the FMCW radar.

We can now analyze the performance impact of the updated OFDM parameters on the communication system. The power spectral density of the transmitted waveform is included in Figure \ref{fig::updated_ofdm_psd}.
...
The radar operates on LOS reflections generated with a fine antenna beamwidth. Therefore, the JRC communication system will operate on channel with little to no multi-path components. As a result, we treat the communication channel as roughly AWGN and only analyze the AWGN performance.

The BER curve using the updated OFDM parameters and assuming an AWGN channel is included in Figure \ref{fig::updated_ofdm_ber}
...
Finally, we can determine the effect of the OFDM parameters on channel capacity. We again assume an AWGN channel. For a fixed signal power, the SNR will be lower when the bandwidth is larger because we have a greater noise bandwidth and a fixed noise power spectral density. This can be visualized by expressing capacity in the following form:

\begin{equation}
	C = B\text{log}_2\left(1 + \frac{P}{{N_0}B} \right)
\end{equation}

We now consider values of $\frac{P}{N_0}$ that achieve different SNRs with the 10 MHz bandwidth of the 802.11p system. For these values of $\frac{P}{N_0}$, we have examined how increasing the bandwidth affects the channel capacity. Figure \ref{fig::capacity_vs_bandwidth} shows the results of this experiment along with the corresponding SNRs assuming a 10 MHz bandwidth.
...
In this paper, we presented a JRC implementation, which used an OFDM 802.11p transmitter and zero forcing to generate RDMs from OFDM returns. We started by providing background and performance results for an FMCW radar. Then, we provided background and performance results for the OFDM system. Finally, we detailed our JRC implementation and compared its performance to the performance of the stand-alone systems.

We observed that performance of the radar degraded with range and uncompensated doppler shifts. Despite this degradation, we inferred that the OFDM radar would provide acceptable PSLR performance over ranges and velocities encountered in an automotive environment. We also observed a degradation in SNR due to the high PAPR and cyclic prefix of OFDM symbols. Finally, we found that the range resolution of the 802.11p radar was significantly worse than the FMCW radar, due to its low bandwidth. This makes the 802.11p radar impractical for automotive radar use.

To achieve better range resolution, we also experimented with a 320 MHz bandwidth JRC system operating in the 77 GHz band. This system provided a sufficient range resolution and did not degrade the OFDM performance in an AWGN channel.

With a reduction in the OFDM PAPR and an increase in the channel bandwidth, we believe that the presented JRC system will be able to sufficiently meet both radar and communication objectives, while reducing total bandwidth usage, power consumption, and the amount of required hardware.
