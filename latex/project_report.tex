...
In this section, we evaluate the performance of an FMCW radar written in MATLAB and configured with the following set of parameters:
	...
	If we input a signal with an SNR of 20 dB and measure the SNR in the RDM, we obtain a value of 70.01 dB, which is consistent with the expected signal processing gain.
	 ...
	 \subsection {Background}
		...
		To deal with the effects of a multi-path, a cyclic prefix is added to each of the OFDM symbols. The cyclic prefix appends the last L samples of an OFDM symbol to the beginning of the OFDM symbol as illustrated in Figure \ref{fig::cylic_prefix}.
		...
		With the cyclic prefix, the receiver receives a circularly-shifted copy of the symbol for each of the signal paths. If we take an FFT at the receiver, each sample of the FFT will be a copy of the transmitted FFT samples weighted by a complex number, which results from the sum of the multi-path components. We can remove this complex weighting by equalizing the received symbol. After equalization, we should be obtain to correctly reconstruct the transmitted symbol.
