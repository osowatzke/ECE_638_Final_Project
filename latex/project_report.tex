...
In this section, we evaluate the performance of an FMCW radar written in MATLAB and configured with the following set of parameters:
	...
	Metrics of interest include the range resolution, peak sidelobe ratio (PSLR), unambiguous range, unambiguous velocity, and received SNR.
	...
	Note that the SNR in the RDM is greater than the input SNR by a factor of $G_{sp}$, where $G_{sp}$ is the SNR gain due to signal processing.
